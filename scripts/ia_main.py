...
def enrich_recipe(
    recipe: Dict[str, Any], ai_service: AIService, global_context: str = ""
) -> Dict[str, Any]:
    """
...
NO agregues campos nuevos. SOLO devuelve estos 3 campos.
    ...
    if result:
        ...
        print("  ⚠️  No se pudo extraer JSON válido")
        return recipe
...
def process_recipes(
    ...
    print(
        f"🔍 Modo: {'DRY RUN (solo visualización)' if dry_run else 'Guardar cambios'}"
    )
    ...
    if not ai_service.verificar_modelo_disponible():
        ...
        print(
            f"📋 Modelos disponibles: {', '.join(ai_service.listar_modelos_disponibles())}"
        )
    ...
    if dry_run:
        ...
        print("   Para guardar, ejecuta sin --dry-run")
    else:
        # Guardar resultado
