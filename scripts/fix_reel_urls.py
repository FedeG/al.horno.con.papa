...
def fix_reel_urls():
    ...
    for i, recipe in enumerate(recipes):
        ...
        if '/p/' in instagram_url:
            ...
            print(f"  ⏭️  [{i+1}/{len(recipes)}] {recipe['name']}: Ya es reel, omitiendo")
            continue
        else:
            print(f"  ⚠️  [{i+1}/{len(recipes)}] {recipe['name']}: URL no reconocida")
            continue
        
        print(f"  🔍 [{i+1}/{len(recipes)}] Verificando {recipe['name']} ({shortcode})...")
        ...
        try:
            ...
            if post.is_video:
                ...
                print(f"  ℹ️  Es un post de imagen, no se modifica")
